...
from demoapp.models import StatusTagEnum, MessageViewDTO

class MessageList:
    def __init__(self):
        self.messages: list[MessageViewDTO] = []
        self.version: int = 0
    ...
    async def on_change(self, message_info: MessageViewDTO):
        await self._onchange.send(sender=self, message_info=message_info)

    async def append(self, message_info: MessageViewDTO):
        self.version += 1
        message_info.version = self.version
        self.messages.append(message_info)
        await self.on_change(message_info)

    def get_by_id(self, id: str) -> MessageViewDTO:
        return next((x for x in self.messages if x.message.id == id), None)

    def get_after_version(self, last_version: int = -1) -> Iterable[MessageViewDTO]:
        return (x for x in self.messages if x.version > last_version)

    async def update_status(self, id: str, tag: StatusTagEnum, value: bool) -> MessageViewDTO:
